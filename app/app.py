...
try:
    ...
    st.sidebar.metric("⏱️ Uptime", health['uptime'])
    ...
    if is_retraining:
        ...
        st.sidebar.info(f"Last Status: {retrain_status['last_status']}")

    with st.sidebar.expander("📝 Live System Logs", expanded=False):
        logs = root_info.get('recent_logs', [])
        log_content = "\n".join(logs) if logs else "Waiting for logs..."
        st.markdown(f'<div class="log-text">{log_content}</div>', unsafe_allow_html=True)
                    ...
                    if resp.status_code == 200:
                        ...
                        with st.container():
                            ...
                            if data.get('is_uncertain'):
                                st.warning("⚠️ Uncertainty Detected")
...
# 3. MLOps Tab
with tab_mlops:
    st.header("⚙️ MLOps Lifecycle Management")
